...
def naive_softmax_loss_and_gradient(
    ...
    loss = -1 * np.log(softmax(matmul)[outside_word_idx])
    expected = np.sum([softmax(matmul)[i] * outside_vectors[i] for i in range(len(outside_vectors))], axis=0)
    grad_center_vec = -1 * (outside_vectors[outside_word_idx] - expected)

    grad_outside_vecs = []
    for i, u in enumerate(outside_vectors):
        y = softmax(matmul)[i]
        if i == outside_word_idx:
            grad_outside_vecs.append(-1 * (center_word_vec * (1 - y)))
        else:
            grad_outside_vecs.append(-1 * -1 * center_word_vec * y)
    ### END YOUR CODE
...
def skipgram(current_center_word, outside_words, word2ind,
    ...
    for word in outside_words:
        ...
        word_loss, word_grad_center_vecs, word_grad_outside_vectors = word2vec_loss_and_gradient(center_word_vec,
                                                                                                 outside_word_idx,
                                                                                                 outside_vectors,
                                                                                                 dataset)
        loss += word_loss
        grad_center_vecs[word2ind[current_center_word]] += word_grad_center_vecs
        grad_outside_vectors += word_grad_outside_vectors
    ### END YOUR CODE
    # print("loss: ", loss)
